...
export const api = {
  ...
  // Market data (market overview page)
  fetchMarketIndices: () => request<IndicesResponse>("/api/market/indices"),
  fetchMarketStocks: (group: "robot" | "ai-compute") =>
    request<StocksResponse>(`/api/market/stocks/${group}`),
  fetchStockKlines: (code: string, type = "daily", count = 60) =>
    request<KlinesResponse>(`/api/market/klines/${code}?type=${type}&count=${count}`),
  fetchStockQuotes: (code: string) =>
    request<QuotesResponse>(`/api/market/quotes/${code}`),

  // Robot research data
  fetchRobotResearch: () =>
    request<RobotResearchResponse>("/api/market/robot-research"),
  refreshRobotResearch: () =>
    request<{ status: string; message: string }>("/api/market/robot-research/refresh", {
      method: "POST",
    }),
...
export interface QuoteData {
  price: number | null;
  servertime: string;
  bid1: number | null; ask1: number | null;
  bid2: number | null; ask2: number | null;
  bid3: number | null; ask3: number | null;
  bid4: number | null; ask4: number | null;
  bid5: number | null; ask5: number | null;
  bid_vol1: number | null; ask_vol1: number | null;
  bid_vol2: number | null; ask_vol2: number | null;
  bid_vol3: number | null; ask_vol3: number | null;
  bid_vol4: number | null; ask_vol4: number | null;
  bid_vol5: number | null; ask_vol5: number | null;
}
...
export interface RobotResearchResponse {
  status: string;
  meta: ResearchMeta;
